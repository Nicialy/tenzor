...
def second():
    ...
    d={rgb_to_name(rgb1):rgb1,rgb_to_name(rgb2):rgb2,rgb_to_name(rgb3):rgb3,rgb_to_name(rgb4):rgb4,rgb_to_name(rgb5):rgb5,rgb_to_name(rgb6):rgb6}
    print(d)
...
def third():
    ...
    print(f'Входят только в первое, но не входят во второе: {s1.difference(s2)}')
    print(f'Входят только во второе, но не входят в первое: { s2.difference(s1)}')
    print(f'Входят в первое или во второе, но не в оба из них одновременно {s1.symmetric_difference(s2)}')
...
def inventory():
      ...
      while True:
            ...
            if dvig.find('1')!=-1:
                ...
                value = float(addinv)
                if value <= 0:
                    print("Вес предмета не может быть отрицательным, у нас тут нету экзотической материи")
                else:
                    if (now+value) > maxweight:
                        print("Инвентарь будет переполнен!!!!!!!!!11111 \n Чтобы улучшить инвентарь повысьте его урвоень!!")
                    else:
